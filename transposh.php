...
class transposh_plugin {
	...
	function transposh_plugin() {
		...
		add_filter( 'plugin_action_links_' . $this->transposh_plugin_basename, array(
			&$this,
			'plugin_action_links'
		) );
		...
		add_filter( 'comment_post_redirect', array(
			&$this,
			'comment_post_redirect_filter'
		) );
		add_filter( 'comment_text', array(
			&$this,
			'comment_text_wrap'
		), 9999 ); // this is a late filter...
		add_action( 'init', array(
			&$this,
			'on_init'
		), 0 ); // really high priority
		//        add_action('admin_init', array(&$this, 'on_admin_init')); might use to mark where not to work?
		add_action( 'parse_request', array(
			&$this,
			'on_parse_request'
		), 0 ); // should have high enough priority
		...
		add_action( 'wp_ajax_tp_tp', array(
			&$this,
			'on_ajax_nopriv_tp_tp'
		) ); // translate suggest proxy
		add_action( 'wp_ajax_nopriv_tp_tp', array(
			&$this,
			'on_ajax_nopriv_tp_tp'
		) );
		add_action( 'wp_ajax_tp_oht', array(
			&$this,
			'on_ajax_nopriv_tp_oht'
		) );
		add_action( 'wp_ajax_nopriv_tp_oht', array(
			&$this,
			'on_ajax_nopriv_tp_oht'
		) );
		// ajax actions in editor
		// TODO - remove some for non translators
		add_action( 'wp_ajax_tp_history', array(
			&$this,
			'on_ajax_nopriv_tp_history'
		) );
		add_action( 'wp_ajax_nopriv_tp_history', array(
			&$this,
			'on_ajax_nopriv_tp_history'
		) );
		add_action( 'wp_ajax_tp_translation', array(
			&$this,
			'on_ajax_nopriv_tp_translation'
		) );
		add_action( 'wp_ajax_nopriv_tp_translation', array(
			&$this,
			'on_ajax_nopriv_tp_translation'
		) );
		add_action( 'wp_ajax_tp_ohtcallback', array(
			&$this,
			'on_ajax_nopriv_tp_ohtcallback'
		) );
		add_action( 'wp_ajax_nopriv_tp_ohtcallback', array(
			&$this,
			'on_ajax_nopriv_tp_ohtcallback'
		) );
		add_action( 'wp_ajax_tp_trans_alts', array(
			&$this,
			'on_ajax_nopriv_tp_trans_alts'
		) );
		add_action( 'wp_ajax_nopriv_tp_trans_alts', array(
			&$this,
			'on_ajax_nopriv_tp_trans_alts'
		) );
		add_action( 'wp_ajax_tp_cookie', array(
			&$this,
			'on_ajax_nopriv_tp_cookie'
		) );
		add_action( 'wp_ajax_nopriv_tp_cookie', array(
			&$this,
			'on_ajax_nopriv_tp_cookie'
		) );
		add_action( 'wp_ajax_tp_cookie_bck', array(
			&$this,
			'on_ajax_nopriv_tp_cookie_bck'
		) );
		add_action( 'wp_ajax_nopriv_tp_cookie_bck', array(
			&$this,
			'on_ajax_nopriv_tp_cookie_bck'
		) );
		...
		if ( $this->options->enable_superproxy ) {
			add_action( 'wp_ajax_proxy', array(
				&$this,
				'on_ajax_nopriv_proxy'
			) );
			add_action( 'wp_ajax_nopriv_proxy', array(
				&$this,
				'on_ajax_nopriv_proxy'
			) );
		}
	...
	function rewrite_url( $href ) {
		...
		// first cut home
		if ( $this->options->enable_url_translate ) {
			$href = transposh_utils::translate_url( $href, $this->home_url, $this->target_language, array(
				&$this->database,
				'fetch_translation'
			) );
		}
	...
	function process_page( $buffer ) { //php7?
		...
		elseif ( $this->options->is_default_language( $this->target_language ) && ! $this->options->enable_default_translate ) {
			...
			$parse                          = new parser();
			$parse->fetch_translate_func    = array(
				&$this->database,
				'fetch_translation'
			);
			$parse->prefetch_translate_func = array(
				&$this->database,
	...
	function is_special_page( $url ) {
		return ( stripos( $url, '/wp-login.php' ) !== false || stripos( $url, '/wp-admin/' ) !== false || stripos( $url, '/wp-comments-post' ) !== false || stripos( $url, '/main-sitemap.xsl' ) !== false || //YOAST?
	...
	function on_parse_request( $wp ) {
		...
		if ( ( $this->options->enable_detect_redirect || $this->options->widget_allow_set_deflang ) && ! ( $this->is_special_page( $_SERVER['REQUEST_URI'] ) || ( isset( $_SERVER['HTTP_REFERER'] ) && strpos( $_SERVER['HTTP_REFERER'], $this->home_url ) !== false ) ) && ! ( transposh_utils::is_bot() ) ) {
		...
		if ( isset( $wp->query_vars['s'] ) ) {
			if ( $this->options->enable_search_translate ) {
				...
				add_action( 'posts_where_request', array(
					&$this,
					'posts_where_request'
				) );
			}
	...
	function get_clean_url() {
		...
		if ( $this->options->enable_url_translate ) {
			$this->clean_url = transposh_utils::get_original_url( $this->clean_url, '', $this->target_language, array(
				$this->database,
				'fetch_original'
			) );
		}
	...
	function request_filter( $query ) {
		...
		if ( $lang && ! $this->got_request ) {
			...
			$_SERVER['REQUEST_URI'] = transposh_utils::get_original_url( $requri, '', $lang, array(
				$this->database,
				'fetch_original'
			) );
			global $wp;
	...
	function tp_shortcode( $atts, $content = null ) {
		...
		if ( $lang || $only_class || $nt_class ) {
			...
			// Tweaked by Heiko
			// Using <br title="TP"> as placeholder
			// The placeholders will striped in functions.php => tp_unautop()
			$newcontent = str_replace( '<p>', '<p><span' . $only_class . $nt_class . $lang . '>', $newcontent );
			$newcontent = str_replace( '</p>', '</span></p>', $newcontent );
			return '<br title="TP"><span' . $only_class . $nt_class . $lang . '><br title="TP">' . $newcontent . '</span><br title="TP">';
		} else {
		...
		foreach ( $urls as $gurl ) {
			if ( $googlemethod < $attempt && $failed ) {
				...
				if ( $info['http_code'] != 200 ) {
					...
					$failed = true;
					update_option( TRANSPOSH_OPTIONS_GOOGLEPROXY, array(
						$attempt,
						time()
					) );
				}
	...
	function get_yandex_translation( $tl, $sl, $q ) {
		...
		if ( $sid == '' ) {
			if ( ( time() - TRANSPOSH_YANDEXPROXY_DELAY > $timestamp ) ) {
				...
				update_option( TRANSPOSH_OPTIONS_YANDEXPROXY, array(
					$sid,
					time()
				) );
				curl_close( $ch );
	...
	function check_for_plugin_update( $checked_data ) {
		...
		$args           = array(
			...
			'version' => '0.9.9.1',
			//$checked_data->checked[$this->transposh_file_location],
		);
